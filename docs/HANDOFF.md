...
- **Active milestone:** M3 accepted and merged to main (PR #3). M4 (Botanical) built on branch `m4-botanical`, PR open, awaiting founder acceptance and merge — this is the first milestone with a real, subjective acceptance criterion ("the founder wants to save a piece"); expect a live-tuning follow-up pass before it's truly "done," not just merged. M5 (sessions and saving) is next pending founder approval
...
**Next session should:**
- **Do a dedicated live-visual tuning pass with the founder before treating M4 as truly accepted** (PR merge and "founder wants to save a piece" are two different bars — this session only clears the first). Candidates to retune based on what's seen live: branch/root density and `MAX_GENERATION` (reference shows denser fanning), blossom opacity/radius ranges (aim for more visible layered translucency, less solid-blob read), growth pace (may want faster for demo purposes), palette exact hue/spread values per preset.
- After that pass (and founder PR review/merge), start M5 (sessions and saving): proper session start/end UI, recipe persistence to IndexedDB, image export, recipe export/import, session-level parameter derivation — building directly on `live-render-loop.ts` rather than re-deriving live-tick-pump logic. Check the founder backlog (4th+ instantaneous input; stillness-speed jitter tuning) before proposing scope.
