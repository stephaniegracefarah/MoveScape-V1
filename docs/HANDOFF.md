...
- **FOUNDER ACTION NEEDED — READ FIRST: the "static before motion" review gate is live.** Branch `m4-botanical-rebuild` has the full structural rebuild (strokes, ramification, blossom clusters, curated palette, depth echoes, Scroll composition) landed and green, and a static seeded render was published as a Claude Artifact for founder review — find the link via `Artifact` tool `action: "list"` if it's not already in hand, or ask the coordinator's session for it. **Per `docs/styles/botanical.md`'s own rule, no motion-mapping retuning happens until the founder looks at that render and says the static composition/mark vocabulary reads right.** See session 008 below for exactly what's in it and what to look for.
...
- **M4 rebuild: structural pass complete on `m4-botanical-rebuild` (not yet a PR — opens once the static-render gate above passes and movement-mapping is retuned).** `branch.ts`/`blossom.ts`/`palettes.ts` rebuilt to `docs/styles/botanical.md`; `botanical.ts` fully rewired (Scroll composition, paletteIndex knob, two depth-echo systems); `tuning-config.ts` fully replaced. 192/192 tests passing, lint/typecheck/build all green. See session 008 for the full breakdown and the real bugs found/fixed while iterating the static render (a fixed windAngle pull fighting the new sweepTarget pull caused spiral artifacts; targetLengthBase was too short for branches to sweep across the canvas; sweepStrength needed rebalancing against the longer targetLengthBase).
- **Works right now:** M0-M3 (webcam adapter, seed/world layer, fixed-timestep engine, compositor incl. strokes/paper-ground) as before. Botanical is mid-rebuild on the feature branch — main still has the pre-rebuild circle-chain/hue-based Botanical until this branch merges. The M4x tuning panel (PR #5/#6, merged to main; also updated on the rebuild branch for the new tuning-config field set) is the tool for the founder's own live retuning pass once the static gate passes.
