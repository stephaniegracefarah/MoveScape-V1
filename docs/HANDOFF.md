...
- **Static-before-motion gate: PASSED.** The founder reviewed the "First Light" static-render artifact and loved it, with one note (see below) — the composition/mark vocabulary is approved. Movement-mapping retuning is now unblocked.
- **Permanent-ink rewrite: DONE (session 009).** The founder's only note on First Light was that the disappearing element should go entirely — recorded as a formal decision in `docs/styles/botanical.md` section 7 ("Persistence") and folded into `m4-botanical-rebuild` ahead of the movement-mapping retune. The `'shrinking'` lifecycle state, `computeShrinkDurationMs`, `visibleSegmentCount`, and the `shrinkRate` tuning field are gone entirely; a mature generation-0 branch now spawns a new sibling at the same root when its timer elapses (front-driven growth) instead of shrinking away and resprouting in its place — nothing is ever removed from a growth system. Total element count still stays bounded by `maxConcurrentBranches` (both forking and resprouting stop once a system hits its cap). 187/187 tests passing (5 dead shrink-specific tests removed), lint/typecheck/build all green. A second static render confirmed the permanent version reads as denser and more accumulated, not chaotic.
...
- **M4 rebuild: structural pass + permanent-ink rewrite complete on `m4-botanical-rebuild` (not yet a PR — opens once movement-mapping is retuned).** `branch.ts`/`blossom.ts`/`palettes.ts` rebuilt to `docs/styles/botanical.md`; `botanical.ts` fully rewired (Scroll composition, paletteIndex knob, two depth-echo systems, permanent lifecycle); `tuning-config.ts` fully replaced. See sessions 008-009 for the full breakdown, the real bugs found/fixed while iterating the static renders, and the permanence rewrite's specifics.
- **Works right now:** M0-M3 (webcam adapter, seed/world layer, fixed-timestep engine, compositor incl. strokes/paper-ground) as before. Botanical is mid-rebuild on the feature branch — main still has the pre-rebuild circle-chain/hue-based Botanical until this branch merges. The M4x tuning panel (PR #5/#6, merged to main; also updated on the rebuild branch for the new tuning-config field set) is the tool for the founder's own live retuning pass once movement-mapping is retuned.
- **Next: movement-mapping retune**, then open the PR for `m4-botanical-rebuild`.
...
**QA notes:**
- Independently reran `lint`/`typecheck`/`test`/`build` after the rewrite (187/187, down from 192 — 5 dead-code tests removed, no unexpected failures) and grepped the built `dist/` bundle (clean, same as every prior session).
...
**Next session should:**
1. Movement-mapping retune (speed→growth rate, expansion→spread/cluster size, symmetry→curvature calm/wild — roles unchanged per visual spec section 7), confirm the immediacy budget invariant still holds with stroke-based rendering and permanent marks.
2. Rerun the full check suite, then open the PR for `m4-botanical-rebuild`.
3. The founder backlog (4th+ instantaneous input; stillness-speed jitter retune) is unchanged and still awaits separate founder approval.
4. After the PR merges: M5 (session start/end UI, recipe persistence, the actual expanding-scroll/portrait-crop/journey-export infrastructure per SPEC.md's M4-vs-M5 split) — now more clearly motivated than ever, since permanent ink means the canvas genuinely needs to expand rather than stay a fixed, eventually-full frame.
