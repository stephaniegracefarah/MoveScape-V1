...
- **M4 rebuild: fully done on `m4-botanical-rebuild`, PR opening next.** `branch.ts`/`blossom.ts`/`palettes.ts` rebuilt to `docs/styles/botanical.md`; `botanical.ts` fully rewired (Scroll composition, paletteIndex knob, two depth-echo systems, permanent lifecycle, expansion-scaled cluster size); `tuning-config.ts` fully replaced. See sessions 008-009 for the full breakdown.
- **Works right now:** M0-M3 (webcam adapter, seed/world layer, fixed-timestep engine, compositor incl. strokes/paper-ground) as before. Botanical is mid-rebuild on the feature branch — main still has the pre-rebuild circle-chain/hue-based Botanical until this branch merges. The M4x tuning panel (PR #5/#6, merged to main; also updated on the rebuild branch for the new tuning-config field set) is the tool for the founder's own live retuning pass once the PR merges.
- **Next: open the PR for `m4-botanical-rebuild`**, then M5 once it merges.
