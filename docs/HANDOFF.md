...
- **Active milestone: the Botanical rebuild to `docs/styles/botanical.md`.** SPEC.md has been folded (per-style visual-spec tier + the Scroll composition decision, with the M4-vs-M5 scope split recorded there). Sequencing: M3 extension (stroke primitive + paper ground) → M4 rebuild (branch topology, blossoms, palette, depth echoes, composition) → a hard "static before motion" founder-review gate → movement-mapping retune → M5. Full plan: `docs/HANDOFF.md` session 007 entry below.
- **M3 extension done: PR #7 (`m3x-stroke-paper`), awaiting founder review/merge.** `SceneElement` is now a discriminated union (`CircleElement | StrokeElement`); the compositor can render tapered strokes and ring-outlined circles; a new seeded `renderPaperGround` replaces the old blank-canvas clear. Botanical's own rendering is untouched so far (still circle-chain branches) — this PR is purely the compositor primitives the M4 rebuild needs next. 170/170 tests passing.
- **M4 rebuild (branch topology/blossoms/palette/depth/composition) not yet started** — next step once PR #7 is reviewed. See session 007's plan for the full breakdown (three parallel builders + coordinator integration + the static-render founder-review gate before any movement-mapping retuning).
- **Works right now:** M0-M4 (webcam adapter, seed/world layer, fixed-timestep engine, compositor, Botanical) as before, plus the M4x tuning panel (PR #5/#6, merged) — see prior entries for the full description. The tuning panel remains the right tool for calibrating the *rebuilt* Botanical once M4's rebuild lands; it does not need to be rebuilt itself.
