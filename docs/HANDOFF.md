...
- **M5 (Sessions and Saving) started, session 010.** Plan approved by the founder: three stages, one PR per stage. Stage 1 (recipe persistence to IndexedDB + export/import, session-level parameter derivation feeding Botanical) is in progress on `m5-storage-session-params`. Stage 2 (expanding canvas) and Stage 3 (session start/end UI, style picker, save/export wiring) follow as separate PRs — see the new "Open deviations" entry below for a scope change to Stage 2 (portrait-crop paused).
- **Next: finish M5 Stage 1**, then Stage 2, then Stage 3.
