...
**Per-style visual specs are a standing tier.** Every style gets its own `docs/styles/<style>.md` as the authority on its concrete look — measurable properties (marks, palette, composition, depth), not vibes — supplementing this document's architectural description. `docs/styles/botanical.md`'s eight-section structure (marks, ramification, blossoms/detail, palette, depth, composition, movement mapping, acceptance test) is the template for every style that follows. A style milestone's acceptance criterion is "passes its visual spec's acceptance test," not a bespoke subjective description written into this document per style.

**Style #1 is Botanical**, inspired by Holger Lippmann's *Recursive Tree X* (by way of Duane Millar Barlow's tree-study): branching structures carrying clusters of overlapping translucent circles as blossoms, with depth expressed through layering — pale, faded sprays in the background and dark, saturated branches in the foreground. It is built fresh; the POC contributes design findings worth re-implementing on their merits: a branch lifecycle (growing → mature → shrinking → resprout) lets a piece live indefinitely; wander scaled by distance-grown keeps curvature constant regardless of pace; and blending expansion/symmetry with slow-drifting noise adds unpredictability while speed stays direct. One known design tension to resolve deliberately in the build: the blossom texture (layered translucency, accumulated overlapping marks) wants an *accumulating* canvas, while shrinking branches want clear-and-redraw. The depth-layer architecture below resolves this — permanent marks accumulate on their depth layer while live geometry redraws on its own. Botanical's concrete look — mark vocabulary, ramification, palette, composition — is specified in full in `docs/styles/botanical.md`, per the standing-tier decision above.

**Composition and canvas (decided 2026-08-22): the Scroll.** The canvas is a fixed-height, rightward-expanding scroll, not a static frame — one or two dominant structures sweep left to right as the piece grows, with subordinate structure and fine detail forking off along the way. The growth front lives at the right edge, where current movement acts. The engine continuously composes a fixed-aspect **portrait crop** that must look complete at every moment — this crop is the default save/share artifact — while the full scroll is exportable on request as **the journey**. This decision governs every style's composition, not just Botanical's; `docs/styles/<style>.md` files work out what "dominant structure" and "sweep" mean for their own aesthetic family. The composition/topology consequences (few, intentional origins; coherent sweep direction; real negative space) are a style-milestone concern; the expanding-canvas, portrait-crop-tracking, and journey-export *infrastructure* itself belongs to M5 (Part 4).
...
**Session working rules (founder-set, session 002).** The coordinator coordinates, QAs, commits, and updates the handoff; builder agents write all feature code and are spawned on an explicitly cheaper model than the coordinator (sonnet for features, haiku for mechanical work). Git: a branch per milestone, small commits pushed early and often, a PR per milestone, merged to main only after the founder's acceptance test. Stop and ask the founder at milestone boundaries, before any software install, and before anything risky; work autonomously inside an approved milestone; check the handoff's Founder backlog before proposing scope. The founder is on limited usage credits — the coordinator verifies builder claims cheaply (targeted greps plus running the check suite, not full-file re-reads) and keeps its own token use lean. The founder is a data analyst, not a software engineer: explain findings plainly, and spell out any manual steps (terminal commands, browser tests) one at a time.
...
**M4 — Botanical.** The real style: branching growth with blossom clusters, depth layering (pale far, dark near), lifecycle, movement mapping honoring the immediacy budget. This milestone is where most of the project's total effort belongs, iterated until the art is save-worthy — the experience MVP. *Accept: passes `docs/styles/botanical.md`'s acceptance test (a static, seeded, movement-free render mistakable at arm's length for the reference's family — "static before motion"), plus the determinism test still passing.*

**M5 — Sessions and saving.** Session start/end flow, recipe persistence to IndexedDB, image export, recipe export/import, session-level parameter derivation (duration, stillnessRatio, averageEnergy, movementVariance) feeding Botanical, plus the Scroll's expanding-canvas, portrait-crop-tracking, and journey-export infrastructure (Part 3, "Composition and canvas"). *Accept: a saved recipe re-imported on a fresh profile reproduces the piece exactly.*

**M6 — Second style.** A cosmic-family style implementing the same interface, with its own `docs/styles/<style>.md` visual spec. *Accept: zero changes required in any engine, adapter, world, or compositor module — verified by diff — and passes its own visual spec's acceptance test.*
